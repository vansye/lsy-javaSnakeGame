...
- 持久化路径统一由 `AppPaths` 管理：优先 `%APPDATA%/SnakeGame`，回退 `user.home/.snakegame`。
- 排行榜文件为 `rankings.dat`，用户配置文件为 `user-config.properties`，均不再依赖工程工作目录。
- Hard 模式默认速度为 `120ms`（非 100ms）。
- 技能触发为：`Q/鼠标右键` 加速、`F/鼠标左键` 金身。
- 设置页已支持滚动，且从游戏页进入设置时自动选中当前模式。
